...
class NuscenesDataset(Dataset):
    ...
    def get_input_data(self, sample_info_indices):
        ...
        for i, n in enumerate(sample_info_indices):
            ...
            for j, cam in enumerate(CAMERAS):
                ...
                # ext_intrinsic = torch.from_numpy(np.eye(4)).float()
                # ext_intrinsic[:intrinsic.shape[0], :intrinsic.shape[1]] = intrinsic
                # ego_2_img = ext_intrinsic @ torch.from_numpy(lidar_to_sensor).float()
        ...
        if self.return_lidar:
            lidar_sample['filename']
            lidar_pcl = LidarPointCloud.from_file(
                self.config.DATASET.DATAROOT + lidar_sample['filename']).points
        else:
            lidar_pcl = None

        return images, intrinsics, extrinsics, lidar_ego_2_img , original_img, lidar_pcl
    ...
    def __getitem__(self, index) -> Any:
        ...
        data["image"], data["intrinsics"], data["extrinsics"], data["lidar_ego_2_img"],\
            orig_img, lidar_pcl = self.get_input_data(sample_info_indices)
