...
from prediction.model.feature_extractor import BEVFormerFeatureExtractor


class BEVFormerPredictor(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        self.cfg = cfg
        
        self.feature_width_x = int((self.cfg.LIFT.X_BOUND[1] - self.cfg.LIFT.X_BOUND[0])\
                              /self.cfg.LIFT.X_BOUND[2])
        self.feature_width_y = int((self.cfg.LIFT.Y_BOUND[1] - self.cfg.LIFT.Y_BOUND[0])\
                              /self.cfg.LIFT.Y_BOUND[2])
        self.feature_width_z = int((self.cfg.LIFT.Z_BOUND[1] - self.cfg.LIFT.Z_BOUND[0])\
                              /self.cfg.LIFT.Z_BOUND[2])
        ...
        self.feature_extractor = BEVFormerFeatureExtractor(
                ...
                num_layers = self.cfg.MODEL.FEATURE_EXTRACTOR.N_LAYERS,
                embed_dim = self.cfg.MODEL.FEATURE_EXTRACTOR.EMBED_DIM,
                num_cameras = self.cfg.DATASET.N_CAMERAS,
                grid_shape = (self.feature_width_x, self.feature_width_y,
                              self.feature_width_z),
                grid_ranges = [
                    self.cfg.LIFT.X_BOUND[0], self.cfg.LIFT.Y_BOUND[0],
                    self.cfg.LIFT.Z_BOUND[0], self.cfg.LIFT.X_BOUND[1],
                    self.cfg.LIFT.Y_BOUND[1], self.cfg.LIFT.Z_BOUND[1]
                ],
                sa_num_heads = self.cfg.MODEL.FEATURE_EXTRACTOR.SA_NUM_HEADS,
                sa_num_levels = self.cfg.MODEL.FEATURE_EXTRACTOR.SA_NUM_LEVELS,
                sa_num_points = self.cfg.MODEL.FEATURE_EXTRACTOR.SA_NUM_POINTS,
                sa_dropout = 0.1,
        ...
        segformer_config = SegformerConfig(
            image_size = self.feature_width_x,
    ...
    def forward(self, x, img_metas, future_egomotion):
        output = {}
        start_time = time.time()
                
        # Image feature extraction
        x = self.feature_extractor(x, future_egomotion, img_metas)
